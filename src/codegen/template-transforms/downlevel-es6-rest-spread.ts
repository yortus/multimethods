...
//   - where `__ARGS__` appears exactly once as a whole argument (but any argument order is fine)
export default function downlevelES6RestSpread(template: Template, arity?: number) {
    if (typeof arity === 'number') {
        // NB: factor out magic string '__ARGS__' to one place project-wide
        template = strengthReduceES6RestSpread(template, '__ARGS__', '$', arity);
    }
    else {
        // TODO: dead code now... remove/simplify...
...
//         // ES5 equivalent for initialising the rest argument __ARGS__
